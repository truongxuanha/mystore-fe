    ...
    <main>
      <div className="grid grid-cols-1 sm:grid-cols-4  mx-auto overflow-hidden rounded-md bg-white mt-3">
        <div className="p-4 col-span-2 border rounded-2xl m-5">
          <img className="w-full h-full object-cover" src={infoProduct?.thumbnail} alt={infoProduct?.name} />
        </div>
            ...
            {infoProduct?.quantity > 0 ? (
              <>
                <div className="flex items-center w-24 border">
                  <Button onClick={() => handlePreviousQuantity()} className="bg-colorBody border-r w-7 px-2 font-medium text-xl">
                    -
                  </Button>
                  <input className="w-10 text-center" type="text" value={quantity} />
                  <Button onClick={() => setQuantity((quantity) => quantity + 1)} className="bg-colorBody border-l w-7 px-2 font-medium text-xl">
                    +
                  </Button>
                </div>
                ...
                <div className="flex gap-3">
                  <Link to="/dat-hang">
                    <Button className="bg-red-500 text-white text-xs px-2 py-1 rounded-lg hover:bg-red-300">{texts.order.ORDER_NOW}</Button>
                  </Link>
                  <button
              ...
              <span className="px-2 py-[3px] text-white bg-red-500 rounded-md opacity-70 cursor-not-allowed">Tạm hết hàng</span>
            )}
          </div>
        </div>
      </div>
