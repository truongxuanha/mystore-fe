...
import { MainContent, WrapperFormImport } from "../ImportProduct/styled";
...
const ImportProduct = () => {
  ...
  const handleAddProduct = () => {
    if (selectedProduct !== null) {
      ...
      if (product && !exportData.some((item) => item.productId === product.product_id)) {
        setExportData([
          ...exportData,
          {
  ...
  const handleChangeInput = (index: number, field: keyof ExportDataType, value: string | number) => {
    const updatedData: any = [...exportData];
  ...
  const calculateTotalPrice = () => {
    return exportData.reduce((total, item) => total + calculateProductTotal(item.quantity, item.price), 0);
  };

  const handleRemoveProduct = (productId: number) => {
    setExportData(exportData.filter((data) => data.productId !== productId));
  };
  ...
  return (
    <WrapperFormImport>
      <h2 className="uppercase text-center py-2 text-2xl">Xuất hàng</h2>
      <MainContent>
