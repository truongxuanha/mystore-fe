...
const OrderView = () => {
  ...
  const handleCreateOrder = async () => {
    ...
    const orderSucces = await dispatch(createOrderDetailBillThunk({ items: extractedData, type: typeOrder })).unwrap();
    if (orderSucces.success) {
      navigate(`/order?bill=${orderData.id}&&complete=true`);
  ...
  return (
    <main className="grid grid-cols-2 gap-2 mt-3 container">
      {!para && (
        <>
