...
export const createProductThunk = createAsyncThunk("product/createProduct", async (rest: CreateProductType, { rejectWithValue }) => {
  try {
    const data = await cretaeProduct({ ...rest });
    if (!data.status) {
...
export const editProductThunk = createAsyncThunk("product/editProduct", async (rest: EditProductType, { rejectWithValue }) => {
  try {
    const data = await editProduct({ ...rest });
    return data?.data;
  } catch (err) {
    return rejectWithValue(err);
  }
